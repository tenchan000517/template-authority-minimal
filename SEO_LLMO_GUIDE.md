...
export const metadata: Metadata = {
  ...
  // === OpenGraph (SNS共有時) ===
  openGraph: {
    title: `ページタイトル | ${meta.siteName}`,
    description: "ページの説明文",
    url: `${meta.siteUrl}/page-path`,
...
```typescript
import type { Metadata } from "next";
import { meta, company, getCaseBySlug } from "@/lib/site";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = item.lead || `${item.client}様の事例`;
  ...
  return {
    ...
    openGraph: {
      ...
      locale: "ja_JP",
      type: "article",
      images: item.mainImage ? [{ url: item.mainImage, alt: item.title }] : undefined,
    },
    ...
    // === LLMO対応 ===
    other: {
      "ai:summary": `${item.client}様の事例。${item.lead}`,
      "ai:topics": `事例, 実績, ${item.industry}`,
    },
  };
}
